...
function scalar(source, label) {
    ...
    if (value === '' || /[\s#]/u.test(value))
        throw new Error(`${label} must be one plain YAML scalar`);
    return value;
}
function packageFromSpecifier(specifier, label) {
    if (!MODULE_SPECIFIER.test(specifier))
        throw new Error(`${label} has an invalid Loader module specifier`);
    if (!specifier.startsWith('@')) {
        const [packageName] = specifier.split('/');
        if (packageName === undefined)
            throw new Error(`${label} has no package name`);
        return packageName;
    }
 ...
 */
export function deriveProtectedSystemComponents(manifestPaths) {
    if (manifestPaths.length === 0)
        throw new Error('shipped composition must contain at least one Bundle');

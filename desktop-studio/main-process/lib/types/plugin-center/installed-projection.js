...
import { decodeInstalledPluginListResult, } from '@deepseek-ai/dsh-plugin-center-contracts';
import { evaluateCompatibility, evaluateInstalledActivationCompatibility, } from "./compatibility.js";
...
function observePackage(profileDirectory, installAnchor, packageName) {
    try {
        ...
        if (fromRoot === '..' || fromRoot.startsWith('../') || fromRoot.startsWith('..\\')
            ...
            throw new Error('Bundle patch is outside or missing');
...
function availableUpdate(current, catalog, fingerprint) {
    const target = catalog.preflights.filter(item => item.pluginId === current.pluginId
        && item.packageName === current.packageName
        && item.reviewed
        && item.eligible
        && !item.withdrawn
        && semver.gt(item.version, current.version)).filter(item => evaluateCompatibility({ candidate: item, fingerprint, action: 'update' }).allowed).sort((left, right) => semver.rcompare(left.version, right.version))[0];
    if (target === undefined)
        return null;
...
function runtimeStatus(enabled, failed, requireActiveEntries, evidence, runtime, expectedEntries, expectedClientModules, expectedSkillIds) {
    ...
    if (!enabled)
        return anyObserved ? 'failed' : 'inactive';
    ...
    const entriesActive = expectedEntries.every(expected => runtime.entries.some((entry) => {
        if (entry.entryId !== expected && entry.entryId !== `include:${expected}`)
            return false;
        if (requireActiveEntries)
            return entry.enabled && entry.fiberPhase === 'active';
...
export function deriveInstalledPluginProjection(input) {
...
//# sourceMappingURL=installed-projection.js.map
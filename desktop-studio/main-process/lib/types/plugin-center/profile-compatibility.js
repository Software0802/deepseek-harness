...
import { resolveCompatibilityFingerprint } from "./environment.js";
...
function packageNames(value, label) {
    ...
    if (names.some(name => !PACKAGE_NAME.test(name)))
...
function bundleEntryIds(patch, label) {
    ...
    for (const [index, line] of patch.split(/\r?\n/u).entries()) {
        ...
        if (!ENTRY_ID.test(value))
            throw new Error(`${label}:${String(index + 1)} has an invalid Loader row id`);
        ids.add(value);
    }
    return [...ids].sort();
}
function addFile(hash, label, path, required) {
    if (!existsSync(path)) {
        if (required)
            throw new Error(`selected Profile is missing ${path}`);
...
function installedIdentity(profileDirectory, packageName, enabled, candidates, hash) {
    ...
    const manifest = record(JSON.parse(bytes.toString('utf8')), manifestPath);
    ...
    if (typeof patch !== 'string' || patch === '')
        throw new Error(`${manifestPath} has an invalid dsh.bundle.patch`);
    ...
    if (matches.length > 1)
        throw new Error(`${packageName}@${version} matches multiple catalog plugins`);
    return {
 ...
 */
export function readProfileCompatibilityFingerprint(input) {
    ...
    const manifest = record(JSON.parse(manifestBytes.toString('utf8')), manifestPath);
    const dsh = optionalRecord(manifest['dsh'], `${manifestPath} dsh`);
    ...
    if (overlap !== undefined)
        throw new Error(`${manifestPath} lists ${overlap} as both active and disabled`);
...
//# sourceMappingURL=profile-compatibility.js.map
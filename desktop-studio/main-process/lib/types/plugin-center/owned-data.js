...
import { decodePluginOwnedDataOffer, decodePluginOwnedDataRemovalRequest, decodePluginOwnedDataRemovalResult, decodePluginOwnedDataRetentionRequest, decodePluginOwnedDataRetentionResult, } from '@deepseek-ai/dsh-plugin-center-contracts';
...
function validateRecord(value) {
    ...
    if (!PACKAGE_NAME.test(source['packageName']))
        throw new Error('owned-data authority package name is invalid');
    if (!EXACT_VERSION.test(source['version']))
        throw new Error('owned-data authority version is invalid');
...
export class PluginOwnedDataAuthorityStore {
    ...
    async read(operationId) {
        ...
        if (!metadata.isFile() || metadata.isSymbolicLink())
            throw new Error('owned-data authority must be a regular file');
...
async function rejectSymlinks(path) {
    ...
    if (metadata.isSymbolicLink())
        throw new Error('owned-data deletion refuses symbolic links');
...
export class PluginOwnedDataRemover {
    ...
    async currentOffer() {
        ...
        if (authority === null || authority.pluginId !== operation.operation.pluginId
            || authority.version !== operation.operation.version || authority.declarations.length === 0)
            return null;
    ...
    async remove(value) {
        ...
        if (request.paths.length === 0)
            throw new Error('owned-data deletion requires at least one selected path');
...
//# sourceMappingURL=owned-data.js.map
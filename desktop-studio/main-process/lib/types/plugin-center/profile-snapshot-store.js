...
function decodeProfileIdentity(value) {
    ...
    if (source['profileName'] !== 'web')
        throw new Error('profile snapshot profileName must equal web');
...
export function decodeProfileMutationSnapshot(value) {
    ...
    if (source['schemaVersion'] !== 2)
        throw new Error('profile snapshot schemaVersion must equal 2');
    const snapshotId = stableId(source['snapshotId'], 'snapshotId');
    const operationId = stableId(source['operationId'], 'operationId');
    if (snapshotId !== operationId)
        throw new Error('profile snapshot must be owned by its operation id');
...
export class ProfileSnapshotStore {
    ...
    snapshotDirectory;
    now;
    constructor(profileDirectory, snapshotDirectory, now = () => new Date()) {
    ...
    async restore(expectation) {
        ...
        if (!sameIdentity(snapshot.profileIdentity, expectation.profileIdentity)
            ...
            throw new ProfileSnapshotError('snapshot-root-mismatch', 'profile snapshot identity does not match the journal or current Profile root');
        }
        try {
            for (const file of snapshot.files)
                await authorityTarget(this.profileDirectory, file.path);
        }
        catch (error) {
    ...
    async pruneClosedSnapshots(currentSnapshotId) {
        ...
        for (const entry of entries) {
            if (entry.name === currentSnapshotId || !entry.isDirectory() || !STABLE_ID.test(entry.name))
                continue;
            try {
...
//# sourceMappingURL=profile-snapshot-store.js.map
...
import { installTrustedPackage, removeTrustedPackage, } from "./package-manager.js";
import { reconcileAndValidateInstalledBundle, reconcileAndValidateUninstalledBundle, } from "./profile-installation.js";
import { readInstalledOwnedDataAuthority } from "./installed-projection.js";
...
export function createTrustedManagementRunner(options) {
    return async (request, controls) => {
        ...
        if (request.action === 'update') {
            if (artifact === undefined)
                throw new PluginOperationFailure('internal', 'update artifact was not retained');
        ...
        try {
                ...
                generation = await options.host.restart(`${request.action} ${candidate.pluginId}@${candidate.version}`, async () => {
                    ...
                    try {
                        if (request.action === 'enable' || request.action === 'disable') {
                            ...
                            if (artifact === undefined)
                                throw new Error('verified update artifact is unavailable');
                    ...
                    try {
                        ...
                        if (request.action === 'uninstall') {
                            if (observed !== undefined)
                                throw new Error('removed plugin remains in the Profile projection');
                        }
                        else {
            ...
            try {
                ...
                runtimeEvidence = targetEnabled
                    ? await options.runtimeVerifier.verifyActivationTransition(generation.origin, candidate, priorRuntime, replacedCandidate)
...
//# sourceMappingURL=trusted-management-executor.js.map
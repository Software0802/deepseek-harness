...
import { ARTIFACT_VERIFICATION_REASON_ORDER, decodeArtifactVerificationResult, } from '@deepseek-ai/dsh-plugin-center-contracts';
...
function archivePath(raw) {
    ...
    if (raw.includes('\\'))
        return { path: raw, issue: 'archive-path-traversal' };
    ...
    return new Promise((resolve, reject) => {
        ...
        parser.on('entry', (entry) => {
            ...
            if (decoded.issue !== undefined)
                add(decoded.issue, raw);
            const path = decoded.path;
            if (paths.has(path))
                add('archive-duplicate-entry', path);
            paths.add(path);
            ...
            if (entryCount > MAX_ENTRY_COUNT)
                add('archive-file-count-exceeded', String(entryCount));
            if (unpackedBytes > MAX_UNPACKED_BYTES)
                add('archive-unpacked-size-exceeded', String(unpackedBytes));
 ...
 */
export async function verifyPluginArtifact(input) {
    ...
    if (sha256 !== evidence.sha256)
        add('sha256-mismatch', input.candidate.packageName);
    const integrity = `sha512-${createHash('sha512').update(bytes).digest('base64')}`;
    if (integrity !== evidence.integrity)
        add('integrity-mismatch', input.candidate.packageName);
    let observation;
    try {
    ...
    if (observation.packageManifest === undefined) {
        ...
        try {
            ...
            if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed))
                throw new TypeError('manifest object required');
            manifest = parsed;
        }
        catch {
    ...
    if (manifest !== undefined) {
        ...
        if (typeof manifest.scripts === 'object' && manifest.scripts !== null && !Array.isArray(manifest.scripts)) {
            for (const script of Object.keys(manifest.scripts)) {
                if (INSTALL_LIFECYCLE_SCRIPTS.has(script))
                    add('lifecycle-script-denied', script);
            }
        }
    ...
    if (!observation.paths.has(patchPath) || observation.bundlePatch === undefined) {
        ...
        for (const entryId of input.candidate.expectedEntries) {
            if (!ids.has(entryId))
                add('expected-evidence-missing', entryId);
        }
        for (const moduleName of input.candidate.expectedClientModules) {
            if (!names.has(moduleName))
                add('expected-evidence-missing', moduleName);
        }
    }

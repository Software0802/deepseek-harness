...
import { decodePluginRuntimeEvidence, } from '@deepseek-ai/dsh-plugin-center-contracts';
...
function normalizeEvidence(value, allowLegacyGeneratedIds = false) {
    ...
    if (!Array.isArray(entries))
        throw new Error('runtime inventory entries must be an array');
    ...
    const projectedEntries = decoded.entries.flatMap((entry, index) => {
        if (!GENERATED_LOADER_ENTRY_ID.test(entry.entryId))
            return [entry];
...
function requireUnrelatedContinuity(prior, observed, candidates) {
    for (const entry of prior.entries) {
        if (!isRestartStableEntry(entry))
            continue;
        if (candidateEntry(candidates, entry.entryId))
            continue;
...
export class PluginRuntimeVerifier {
    ...
    createRpcId;
    timeoutMs;
    constructor(fetcher = fetch, createRpcId = randomUUID, timeoutMs = 10_000) {
    ...
    async readEvidence(origin) {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`runtime inventory returned HTTP ${String(response.status)}`);
            const envelope = record(await response.json(), 'runtime inventory response');
            ...
            if (result['ok'] !== true)
                throw new Error('runtime inventory request failed');
    ...
    async verifyActivation(origin, candidate) {
        ...
        for (const expected of candidate.expectedEntries) {
            const active = inventory.entries.some(entry => (entry.entryId === expected || entry.entryId === `include:${expected}`)
                && entry.enabled
                && entry.fiberPhase === 'active');
            if (!active)
                throw new Error(`expected Loader entry is not active: ${expected}`);
        }
    ...
    async verifyDeactivation(origin, candidate, prior, replacedCandidate) {
        ...
        requireUnrelatedContinuity(normalizeEvidence(prior, true).evidence, inventory, replacedCandidate === undefined ? [candidate] : [candidate, replacedCandidate]);
        return inventory;
    }
}

...
import { decodePluginTransactionJournalRecord, } from '@deepseek-ai/dsh-plugin-center-contracts';
...
async function durableAtomicWrite(path, content) {
...
function assertInitialRecord(record) {
    ...
    if (initial?.sequence !== 0 || initial.phase !== 'preflight' || initial.boundary !== 'observation'
        ...
        throw new PluginOperationJournalError('journal-invalid', 'plugin operation journal must begin with one empty preflight observation');
    }
}
...
function decodeStoredJournal(value) {
    if (typeof value === 'object' && value !== null && !Array.isArray(value)
        ...
        throw new PluginOperationJournalError('unsupported-journal-version', 'plugin operation journal version is not supported');
    }
    try {
...
async function pruneTerminalHistory(directory) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            if (`${record.header.operationId}.json` !== entry.name
                || (record.terminalResult !== 'committed' && record.terminalResult !== 'rolled-back'))
                continue;
    ...
    for (const terminal of terminals.slice(MAX_ARCHIVED_JOURNALS))
        await unlink(terminal.filename);
    if (terminals.length > MAX_ARCHIVED_JOURNALS)
...
export class PluginOperationJournal {
    ...
    async write(value) {
        ...
        if (previous === null)
            ...
            await durableAtomicWrite(join(historyDirectory, `${previous.header.operationId}.json`), `${JSON.stringify(previous, null, 2)}\n`);
            await pruneTerminalHistory(historyDirectory);
        }
        else
            assertMonotonic(previous, record);

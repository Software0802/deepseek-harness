...
import { COMPATIBILITY_REASON_ORDER, decodeCompatibilityDecision, } from '@deepseek-ai/dsh-plugin-center-contracts';
...
function actionSupported(action, candidate, installed) {
    if (!candidate.supportedActions.includes(action))
        return false;
    if (action === 'install')
        return installed === undefined;
    if (installed === undefined)
        return false;
    if (installed.packageName !== candidate.packageName)
        return false;
    if (action === 'update') {
...
export function evaluateCompatibility(input) {
    ...
    if (!candidate.reviewed)
        add('catalog-unverified', candidate.pluginId, 'false', 'true');
    if (candidate.withdrawn)
        add('version-withdrawn', candidate.version, 'true', 'false');
    if (!candidate.eligible)
        add('version-ineligible', candidate.version, 'false', 'true');
    ...
    for (const entryId of candidate.expectedEntries) {
        if (fingerprint.protectedEntryIds.includes(entryId))
            add('protected-entry', entryId);
    }
    ...
    if (artifact === undefined)
        add('artifact-missing', fingerprint.platform);
    ...
    for (const plugin of fingerprint.installedPlugins) {
        ...
        if (plugin.pluginId === candidate.pluginId)
            continue;
        for (const entryId of candidate.expectedEntries) {

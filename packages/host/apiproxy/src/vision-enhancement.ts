/**
 * Desktop visual augmentation: a verified vision provider turns images into
 * model-visible text so a text-only agent can still read screenshots and
 * workspace pictures. The default provider is xAI Grok 4.6 using the same
 * `XAI_API_KEY` credential as the chat route.
 */
...
export const XAI_API_KEY_REF = credentialRef('XAI_API_KEY')
...
export const XAI_VISION_MODEL = 'grok-4.6'
export const OPENROUTER_VISION_MODEL = 'openai/gpt-4.1-mini'
export const XAI_API_KEY_URL = 'https://console.x.ai/'
...
- 不要猜测看不清的细节；识别失败时明确说明，并建议用户换清晰图片或重新配置视觉凭证。
...
const VISION_PROVIDER_SPECS: Record<VisionProvider, VisionProviderSpec> = {
  xai: {
    id: 'xai',
    name: 'xAI',
    credentialRef: XAI_API_KEY_REF,
    defaultModel: XAI_VISION_MODEL,
    apiKeyUrl: XAI_API_KEY_URL,
...
const VISION_PROVIDER_ORDER: readonly VisionProvider[] = ['xai', 'openrouter']

const VisionSettingsSchema = z.object({
  enabled: z.boolean().default(false),
  provider: z.union(['xai', 'openrouter']).default('xai'),
  model: z.string().default(XAI_VISION_MODEL),
})
...
/**
 * Persist one visual observation on the session log, racing identical appends.
 * @param session - session that owns the image
 * @param input - identity of the observation without the description
 * @param analyze - provider call that produces the description
...
export async function ensureLoggedVisionObservation(
  ...
  return session.append('vision/observation', { ...input, description }).data.description
}
...
async function boundedJson(response: Response, providerName: string): Promise<Record<string, unknown>> {
...
function resolveVisionSelection(settings: VisionSettings): { provider: VisionProvider; model: string; spec: VisionProviderSpec } {
...
function resolveRequestedSelection(provider: VisionProvider | undefined, model: string | undefined) {
  const spec = VISION_PROVIDER_SPECS[provider ?? 'xai']
...
async function resolveProviderCredential(ctx: Context, spec: VisionProviderSpec): Promise<string | undefined> {
  ...
  if (spec.fallbackCredentialRef === undefined) return undefined
...
async function providerConfigured(ctx: Context, spec: VisionProviderSpec): Promise<boolean> {
  ...
  if (spec.fallbackCredentialRef === undefined) return false
  return (await ctx.credentials.describe(spec.fallbackCredentialRef)).configured
}

async function visionAnalyze(
  ctx: Context,
  selection: { provider: VisionProvider; model: string; spec: VisionProviderSpec },
  ...
  if (credential === undefined) throw new Error(`尚未配置 ${selection.spec.name} 凭证。`)
  ...
  if (!response.ok) {
    ...
    throw new Error(error?.message ?? (typeof payload.message === 'string' ? payload.message : undefined)
      ?? `${selection.spec.name} 视觉服务请求失败（HTTP ${response.status}）。`)
  }
  const choices = payload.choices as Array<{ message?: { content?: unknown } }> | undefined
  const description = contentText(choices?.[0]?.message?.content)
...
async function transformBlocks(
  ...
  question: string,
  model: string,
  analyze: (attachment: ImageAttachmentRef, question: string, signal?: AbortSignal) => Promise<string>,
  signal?: AbortSignal,
...
/**
 * Install settings, the vision tool/skill, and the text-model image bridge.
 * @param ctx - host context with credentials, settings, attachments, and tools
 * @returns runtime used by the ApiProxy vision methods
 */
export function installVisionEnhancement(ctx: Context): VisionEnhancementRuntime {
  ...
  const visionTool = defineTool({
    ...
    async execute(args, exec) {
      if (!current().enabled) throw new Error('视觉能力增强尚未开启，请先在通用设置中完成视觉凭证验证。')
  ...
  installSettingsSection(ctx, VISION_SETTINGS_NAMESPACE, VisionSettingsSchema, { enabled: false, provider: 'xai', model: XAI_VISION_MODEL }, {
    setSource: (source) => { current = () => source() as VisionSettings },
  ...
  ctx.on('llm/stream', (options, next) => {
    ...
    return (async function* () {
      ...
      const messages = await transformMessages(
        options.messages,
        selection.model,
        (attachment, question, signal) => analyzeAttachment(agent.session, attachment, question, selection, signal),
        options.signal,
      )
  ...
  return {
    ...
    async status() {
      const selection = resolveVisionSelection(current())
      const providers = await Promise.all(VISION_PROVIDER_ORDER.map(async (id) => {

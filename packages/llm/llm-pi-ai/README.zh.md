...
catalog 条目提供订阅登录的路由（已安装的 xAI SuperGrok / X Premium 登录即属此类）可以设置 `auth: oauth`：harness 会自行运行提供方的设备码登录，并把得到的凭据存到 `apiKeyEnv` 之下——存储形式是请求路径在过期时会自动刷新的 JSON 文档，因此 token 与有效期都无需任何配置。`oauth` 要求提供 `apiKeyEnv`（存储引用）且 catalog 条目确实提供登录；默认值是 `api-key`，除非该 catalog 只提供订阅登录。
...
受支持的 profile 字段是 `apiKeyEnv`、`auth`、`displayName`、`api`、`baseURL`、`models`、`modelOverrides`、`compat`、`defaultContextWindow`、`defaultMaxTokens`、`defaultInput`、`headers`、`reasoning`、`thinkingBudgets`、`cacheRetention`、`transport`、`timeoutMs`、`websocketConnectTimeoutMs`、`streamIdleTimeoutMs` 和 `retryPolicy`。每个 profile 的可选重试策略都会与该提供方路由一同捕获；省略时使用有界的常规默认值。流空闲间隔必须是正的有限 Node 定时器延迟，默认为五分钟，且只覆盖未完成提供方读取，不包括消费方思考时间。若已配置标头中有同名项，则以 Harness 应用归因为准。

`auth` 选择该路由的认证方式：`api-key` 把密钥存到 `apiKeyEnv` 之下，是 catalog 提供密钥方法时的默认值；`oauth` 运行 catalog 的订阅登录并把凭据存到 `apiKeyEnv` 之下——此时必须提供该引用，且 catalog 条目必须确实提供登录。请求路径即使 profile 仍写着 `api-key` 也会认领已存储的订阅凭据，因此修正 profile 之前完成的登录依然有效。Models 页面会为所有支持订阅登录的 catalog 路由提供登录入口。
...
- **订阅登录运行设备码流程，不支持交互式提示**：catalog 登录若需要提示（`manual_code` 等），将无法完成——harness 交互会拒绝提示。目前内置的每个 catalog 登录都是设备码流程；登录也绝不会存下 pi-ai 环境发现本可找到的 token。
- **刷新发生在请求路径上**：过期的订阅凭据会在请求解析它时被刷新并重新存储；这里没有任何定时刷新，因此刷新 token 已失效的凭据会在发现它的那次请求上失败。并发请求可能各自刷新一次；xAI 的流程能容忍这一点（刷新 token 不轮换）。

...
A route whose catalog entry offers a subscription login (xAI's SuperGrok / X Premium sign-in is the shipped one) can set `auth: oauth`: the harness then runs the provider's own device-code login and stores the resulting credential under `apiKeyEnv` — as a JSON document the request path refreshes when expired, so no token or expiry ever needs configuration. `oauth` requires `apiKeyEnv` (the storage reference) and a catalog entry that actually offers a login; the default is `api-key` unless the catalog offers only a subscription.
      ...
      openai:
        ...
        retryPolicy:
          ...
          backoff:
            ...
            maxDelayMs: 10000
            jitterRatio: 0.1
      # Catalog route authenticated by a subscription login. The login stores
      # its credential under apiKeyEnv; the request path refreshes it.
      xai:
...
Supported profile fields are `apiKeyEnv`, `auth`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, and `retryPolicy`. Each profile's optional retry policy is captured with that provider route; omission uses bounded normal defaults. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. Harness app attribution wins a conflicting configured header name.

`auth` selects how the route authenticates: `api-key` stores a key under `apiKeyEnv` and is the default for a route whose catalog offers one; `oauth` runs the catalog's subscription login and stores its credential under `apiKeyEnv` — the reference is required there, and so is a catalog entry that actually offers a login. The request path honors a stored subscription credential even when the profile still says `api-key`, so a login run before the profile was corrected keeps working. The Models page offers the login itself for every catalog route that supports one.
...
- **Subscription logins run the device flow, not interactive prompts** — a route whose catalog login needs a prompt (`manual_code` and friends) cannot complete: the harness interaction refuses prompts. Every shipped catalog login today is a device-code flow, and the login never stores a token pi-ai's ambient discovery could have found instead.
- **Refresh happens on the request path** — an expired subscription credential is refreshed when a request resolves it, then stored back; nothing here refreshes on a timer, so a credential whose refresh token has lapsed fails the request that discovers it. Concurrent requests may refresh once each; xAI's flow tolerates that (non-rotating refresh tokens).

...
| `/loop` | 挂载 [`@deepseek-ai/dsh-omp-loop`](../../packages/omp/omp-loop/README.md)；人类命令通过 `Agent.followup()` 按次数或时长提交由渲染器拥有的续行，没有完成语义 |
| Advisor（第二模型审阅） | 挂载 [`@deepseek-ai/dsh-omp-advisor`](../../packages/omp/omp-advisor/README.md)；旁路 `ctx.llm.stream` 审阅每个已完成轮次，并用 `inject()` / `followup()` 投递通知 |
